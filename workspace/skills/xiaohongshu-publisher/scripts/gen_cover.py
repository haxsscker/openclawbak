...
def draw_badge(img, text):
    ...
    # Use getsize for older Pillow versions
    try:
        bw, bh_raw = draw.textsize(text, font=font)
    except AttributeError:
        # Fallback for very old versions
        bw, bh_raw = font.getsize(text)
    bw = bw + 50
    bh = bh_raw + 24
    bx, by = W - bw - 50, 60
    ...
    # Use rectangle instead of rounded_rectangle for older versions
    bd.rectangle([bx, by, bx + bw, by + bh],
                 fill=(255, 255, 255, 60), outline=(255, 255, 255, 100))
